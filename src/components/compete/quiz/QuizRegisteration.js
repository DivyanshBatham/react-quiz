...
class QuizRegistration extends Component {
  ...
  render() {
    ...
    // let startTime = +moment(this.props.quizDoc.startTime.toDate()).format("X");
    // let endTime = +moment(this.props.quizDoc.endTime.toDate()).format("X");
    ...
    if (now < startTime) {
      status = "Upcoming";
      timestamp = "Starts " + moment(this.props.quizDoc.startTime.toDate()).fromNow(); // Make this return "Starts in x minutes.."
    } else if (now < endTime) {
      status = "Running";
      timestamp =
        (Math.abs(moment().diff(moment(this.props.quizDoc.endTime.toDate()), "minutes")) + 1) + 
        " minutes remaining";
    } else {
      status = "Finished";
      ...
      <main className={this.props.sideNavActive ? "activeSidenav" : null}>
        <div className="container flex_col">
          <header>
            ...
            <div>
              <div className="quizlist__quiz__detail">
                ...
                {/* {this.props.quizDoc && <>{this.props.quizDoc.registeredUsers.length}</>} */}
          ...
          <button
            ...
            disabled={this.props.quizDoc.registeredUsers.includes(
              this.props.uid
            )}
            onClick={() =>
          ...
          <hr />
          <p>TODO: List of users that have registered for the Quiz.</p>
        </div>
      </main>
    );
  }
